...
function contextSwitch {
	...
	sleep 1
        ...
        ctxt=$(($ctxt2 - $ctxt1))
        result="Number of context switches in the last second: $ctxt"
	echo $result > result
...
function userKernelMode {
	...
	echo 90
        ...
        sum=$(($kernel + $user))
        result="Percentage of last second in usermode: \
        $((( $user*100)/$sum ))% \
...
function interupts {
	...
	ints=$(vmstat 1 2 | tail -1 | awk '{print $11}')
        result="Number of interupts in the last second:  $ints"
	echo 100
	echo $result > result
